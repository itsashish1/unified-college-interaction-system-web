...
export const globalSearch = async (req, res) => {
  try {
    ...
    const [clubs, events, posts, faculty, placements] = await Promise.all([
      ...
      Placement.find({ $or: [{ company: regex }, { role: regex }] }).select('company role type location packageStipend').limit(5),
    ]);

    res.json({ clubs, events, posts, faculty, placements });
